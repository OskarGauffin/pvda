...
using disproportionality analysis, this is handled through the function `da`.

The `pvutils` package contains a simulated data set 'drug_event_df'. For 
...
event are described in the data object (execute `?df_drug_event`). We pass the 
...
The `add_expected_counts`-function has a single parameter, 
`expected_count_estimators` that can be used to turn off calculation of 
...
The `add_disproportionality` function has a `da_estimator` parameter, which
by default includes IC, PRR and ROR. If a subset is preferred, just pass 
the desired estimators in lower case letters.

The `add_disproportionality` function has two parameters. One is the 
`rule_of_N`, by default set to 3, which is sometimes referred to as 
...
The second parameter `number_of_digits` controls the rounding of non-count values in the output, 
