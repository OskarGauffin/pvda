...
collected through spontaneous reporting plays an instrumental part in this 
development.
...
"drug" and "event". If we rename the report-id column in "drug_event_df" 
we need to specify the new name in the call to da, as shown below:
...
# Rule of N
The function `da` calls a function `add_disproportionality` with 
two parameters of interest. One is the `rule_of_N`, by default set to 3, 
...
Another parameter `number_of_digits` controls the rounding of non-count values in the output, 
including all expected counts, uncertainty bounds and point estimates. Note that the printing defaults of dplyr are not overridden in the package, and less digits than expected may be printed. 
However, the additional digits are available and printed if the data frame is
converted to a data frame first, as demonstrated below:
...
any subgroup analysis of e.g. specific age groups can be achieved by filtering out
the group of interest, and then passing it to `da`.  

If a exhaustive grouping is required, one can provide a grouping variable 
`group_by` to `da`, which will execute a call to da for each grouping and 
return the results in a single data frame. The grouping could for instance 
...
be processed by `da` in `pvutils`. Execution time on a regular laptop was less 
than 10 seconds.
