...
summary.da <- function(object, ...){
  ...
  lower_bound_column_names <- split(da_estimators, da_estimators) |>
    lapply(\(x){as.character(colnames_da(quant_prob_list, x)[[1]])}) |>
    purrr::list_c()
  ...
  ic_exp_col <- paste0("2^(", lower_bound_column_names[ic_index], ")", collapse="")
  ...
  message("\n")
  message(cli::col_blue("Top 5 DECs"))
  print(top_five_rows, row.names = FALSE)
  message(cli::col_grey("(sorted according to passed argument sort_by)"))
...
printed_by_default_tbl <- utils::capture.output(print(x$da_df))
...
for(i in seq_along(column_group)){
  ...
  printed_by_default_tbl[2] <- stringr::str_replace(printed_by_default_tbl[2],
                                                    suppressWarnings(stringr::fixed(x$lower)),
                                                    suppressWarnings(stringr::fixed(paste0("{", col, " {x$lower}"))))
  printed_by_default_tbl[2] <- stringr::str_replace(printed_by_default_tbl[2],
...
# Don't want the classes of each col printed
printed_by_default_tbl <- printed_by_default_tbl[-3]
